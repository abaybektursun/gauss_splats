...
int main()
{
    ...
    std::vector<ProjectedGaussian_small> h_splats_debug(vertexCount);
    cudaMemcpy(h_splats_debug.data(), d_outSplats, vertexCount*sizeof(ProjectedGaussian_small), cudaMemcpyDeviceToHost);
    for (int i = 0; i < vertexCount; i++) {
    ...
    std::vector<ProjectedGaussian_small> h_splatsSorted(vertexCount);
    cudaMemcpy(h_splatsSorted.data(), d_outSplats,
               vertexCount*sizeof(ProjectedGaussian_small),
